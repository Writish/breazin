...
struct VideoProjectLoadTests {
    ...
    private func makeBundle(tracks: Int = 1, fileExtension: String = "breazin") throws -> URL {
        let bundle = fm.temporaryDirectory
            .appendingPathComponent("vp-load-\(UUID().uuidString).\(fileExtension)", isDirectory: true)
    ...
    @Test func legacyProjectPackageExtensionStillLoads() throws {
        let bundle = try makeBundle(fileExtension: Project.legacyFileExtension)
        defer { try? fm.removeItem(at: bundle) }

        let contents = try VideoProject.readProjectPackage(at: bundle)

        #expect(contents.projectFile.timelines.count == 1)
    }
    ...
    @Test func missingTimelineStillThrows() throws {
        ...
        let bundle = fm.temporaryDirectory
            .appendingPathComponent("vp-empty-\(UUID().uuidString).breazin", isDirectory: true)
        try fm.createDirectory(at: bundle, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: bundle) }
    ...
    @Test func saveAsPreservesUnreadableManifestFile() throws {
        ...
        let dest = fm.temporaryDirectory
            .appendingPathComponent("vp-dest-\(UUID().uuidString).breazin", isDirectory: true)
