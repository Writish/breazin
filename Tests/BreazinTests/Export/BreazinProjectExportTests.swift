...
@Suite("Breazin project export (collect media)")
struct BreazinProjectExportTests {
    ...
    private func makeFixture() throws -> (root: URL, source: URL, dest: URL, externalContents: String) {
        ...
        return (root, source, root.appendingPathComponent("Packaged.breazin", isDirectory: true), externalContents)
    }
    ...
    @Test func collectsExternalAndInternalRewritesSourcesReportsMissing() throws {
        ...
        let report = try BreazinProjectExporter.export(
