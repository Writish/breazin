...
struct ExportProjectToolTests {
    ...
    @Test func exportsXMLForANonActiveTimelineById() async throws {
        ...
        let breazin = await h.runRaw("export_project", args: ["mode": "breazin", "outputPath": "/tmp/x.breazin", "timelineId": String(other.id.prefix(8))])
        #expect(breazin.isError)
        #expect(ToolHarness.textOf(breazin).contains("breazin"))
    }
    ...
    @Test func exportsXML() async throws {
        ...
        let breazinURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("export-tool-\(UUID().uuidString).breazin")
        defer { try? FileManager.default.removeItem(at: breazinURL) }
        let breazin = try await h.runOK("export_project", args: [
            "mode": "breazin", "outputPath": breazinURL.path,
        ]) as? [String: Any]
        try await waitForJob(from: breazin, in: h.exportQueue)
