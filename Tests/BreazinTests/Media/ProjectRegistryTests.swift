...
struct ProjectRegistryTests {
    ...
    private func makeProjectURL(_ name: String = "Test") -> URL {
        URL(fileURLWithPath: "/tmp/\(name)-\(UUID().uuidString).breazin")
    }
    ...
    @Test func registerTreatsPathsWithExtraSlashesAsTheSameProject() {
        ...
        let reg = makeRegistry()
        let url1 = URL(fileURLWithPath: "/tmp/dedupe-\(UUID().uuidString).breazin")
    ...
    @Test func projectEntryDerivesNameFromURL() {
        let entry = ProjectEntry(
            id: UUID(),
            url: URL(fileURLWithPath: "/tmp/MyProject.breazin"),
            createdDate: Date(),
            lastOpenedDate: Date()
        )
    ...
    @Test func projectEntryIsAccessibleReflectsFileExistence() throws {
        let real = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("entry-\(UUID().uuidString).breazin")
        ...
        let absent = ProjectEntry(
            id: UUID(),
            url: URL(fileURLWithPath: "/tmp/does-not-exist-\(UUID().uuidString).breazin"),
            createdDate: Date(),
            lastOpenedDate: Date()
        )
