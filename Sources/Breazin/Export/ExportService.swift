...
private struct ExportAnalyticsRun {
    ...
    init(breazinContext context: ExportAnalyticsContext) {
        self.basePayload = [
            "source": context.source,
            "project_id": context.projectId ?? "unknown",
            "mode": "breazin",
...
final class ExportService {
    ...
    var lastBreazinReport: BreazinProjectExporter.Report?
    ...
    /// Writes a self-contained `.breazin` bundle (all media collected internally).
    @discardableResult
    func exportBreazinProject(
        ...
        outputURL: URL,
        analyticsContext: ExportAnalyticsContext = .init()
    ) async -> BreazinProjectExporter.Report? {
        reset()
        defer { activeCancellation = nil }
        let analytics = ExportAnalyticsRun(breazinContext: analyticsContext)
        ...
        do {
            ...
            setPhase(.exporting)
            Log.export.notice(
                "breazin export start url=\(outputURL.lastPathComponent)",
                telemetry: "Breazin project export started",
                data: [
            ...
            let worker = Task.detached(priority: .userInitiated) {
                try BreazinProjectExporter.export(
            ...
            lastBreazinReport = report
            didCommitOutput = true
            setProgress(1)
            Log.export.notice(
                "breazin export ok collected=\(report.collected.count) missing=\(report.missing.count)",
                telemetry: "Breazin project export finished",
            ...
            if Self.isCancellation(error) {
                wasCancelled = true
                Log.export.notice("breazin export cancelled", telemetry: "Export cancelled")
            } else {
                self.error = Log.detail(error)
                Log.export.error(
                    "breazin export failed: \(Log.detail(error))",
                    telemetry: "Breazin project export failed",
