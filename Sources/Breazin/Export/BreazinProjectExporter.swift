...
/// Writes a self-contained `.breazin` package: every resolvable media reference is copied
/// into the new bundle's `media/` directory and rewritten to a project-relative source
enum BreazinProjectExporter {
    ...
    static func export(
        ...
        let staging = parent.appendingPathComponent(".breazin-export-\(UUID().uuidString).partial", isDirectory: true)
