...
enum ExportDestination: String, CaseIterable, Identifiable {
    ...
    case breazinProject = "Breazin Project"
...
struct ExportView: View {
    ...
    @State private var breazinSummary: (collect: Int, missing: Int, bytes: Int64) = (0, 0, 0)
    ...
    var body: some View {
        ...
        .task {
            ...
            let summary = await Task.detached(priority: .utility) {
                Self.computeBreazinSummary(entries: entries, projectURL: projectURL)
            }.value
            guard !Task.isCancelled else { return }
            breazinSummary = summary
        }
    }
        ...
        VStack(spacing: AppTheme.Spacing.zero) {
            VStack(alignment: .leading, spacing: AppTheme.Spacing.zero) {
                VStack(alignment: .leading, spacing: AppTheme.Spacing.zero) {
                    ...
                    if editor.timelines.count > 1, destination != .breazinProject {
    ...
    private var breazinProjectSettings: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.xs) {
            Text("Saves a copy of this project with all media bundled inside, so it opens on any machine.")
                .font(.system(size: AppTheme.FontSize.sm))
                .foregroundStyle(AppTheme.Text.secondaryColor)

            if breazinSummary.missing > 0 {
                Text("\(breazinSummary.missing) media file\(breazinSummary.missing == 1 ? "" : "s") missing - they'll be skipped.")
                    .font(.system(size: AppTheme.FontSize.xs))
                    .foregroundStyle(AppTheme.Status.errorColor)
            }
        }
        ...
        return HStack(spacing: AppTheme.Spacing.lg) {
            ...
            case .breazinProject:
                HStack(spacing: AppTheme.Spacing.xs) {
                    ...
                    Text("~\(ByteCountFormatter.string(fromByteCount: breazinSummary.bytes, countStyle: .file))")
                }
    ...
    private var exportFormat: ExportFormat {
        ...
        case .breazinProject: .xml   // Breazin Project has its own path; never rendered.
        case .video: codec.exportFormat
        }
    }

    /// Quick estimate for exporting a Breazin Project
    private nonisolated static func computeBreazinSummary(
    ...
    private func startExport() {
        if destination == .breazinProject { startBreazinExport(); return }
        submissionError = nil
    ...
    private func startBreazinExport() {
        submissionError = nil
        let panel = NSSavePanel()
        ...
        panel.begin { response in
            guard response == .OK, let url = panel.url else { return }
            do {
                try exportQueue.enqueueBreazinProject(
