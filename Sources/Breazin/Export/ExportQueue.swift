...
struct ExportJob: Identifiable, Sendable {
    ...
    var error: String?
    var warnings: [String]
    var breazinReport: BreazinProjectExporter.Report?
}
...
final class ExportQueue {
    ...
    func enqueueBreazinProject(
        ...
        try enqueue(outputURL: outputURL, projectID: projectID, source: source) { service in
            await service.exportBreazinProject(
                projectFile: projectFile,
                manifest: manifest,
                generationLog: generationLog,
    ...
    private func finish(_ id: UUID, status: ExportJobStatus, service: ExportService) {
        ...
        if let report = service.lastBreazinReport {
            jobs[index].breazinReport = report
...
private extension ExportJob {
    func warningCount(_ mediaReport: ExportRunReport?) -> Int {
        if let breazinReport { return breazinReport.missing.count }
        if let mediaReport {
