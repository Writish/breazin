...
/// Reads skills from the environment-specific Breazin directory.
@Observable
@MainActor
final class SkillStore {
    ...
    nonisolated static var directory: URL {
        AppConfiguration.current.skillDirectory
    }
    ...
    /// Resolves `~/.breazin/skills/<id>/` only when `id` is a single safe path component.
    ...
    /// Copies under a `breazin-` prefix so we only overwrite our own prior copy
    @discardableResult
    func copy(_ skill: Skill, to agent: SkillExternalAgent) -> URL? {
        let source = skill.path.deletingLastPathComponent()
        let dest = agent.skillsDirectory.appendingPathComponent("breazin-\(skill.id)", isDirectory: true)
        let fm = FileManager.default
        do {
