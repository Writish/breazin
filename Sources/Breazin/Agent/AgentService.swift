...
final class AgentService {
    ...
    init() {
        ...
        configurationObserver = NotificationCenter.default.addObserver(
            forName: .agentProviderConfigurationChanged,
            object: nil,
            queue: .main
        ) { [weak self] _ in
    ...
    private func reloadProviderConfiguration() {
        Task { [weak self] in
            let keys = await Task.detached(priority: .utility) {
                (AnthropicKeychain.load() ?? "", DeepSeekKeychain.load() ?? "")
            }.value
            self?.anthropicAPIKey = keys.0
            self?.deepSeekAPIKey = keys.1
            self?.provider = AgentProviderConfiguration.selectedProvider
        }
    }
    ...
    var hasApiKey: Bool { !selectedAPIKey.isEmpty }
    ...
    var provider: AgentChatProvider = AgentProviderConfiguration.selectedProvider
