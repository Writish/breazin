...
struct LegacyBackendClient: AgentProvider {
    let providerID: ProviderID = "legacy-backend"
    ...
    private func run(
        ...
        guard let baseURL = BackendConfig.convexHttpURL else {
            throw LegacyBackendClientError.upstream("Backend not configured")
        }
        ...
        guard let session = await Clerk.shared.session, session.status == .active else {
            throw LegacyBackendClientError.unauthenticated
        }
        ...
        if let http = response as? HTTPURLResponse, http.statusCode >= 400 {
            ...
            throw LegacyBackendClientError.from(status: http.statusCode, body: body)
        }
...
enum LegacyBackendClientError: LocalizedError {
    ...
    static func from(status: Int, body: String) -> LegacyBackendClientError {
