...
enum AgentChatProvider: String, CaseIterable, Sendable {
    case anthropic
    ...
    var models: [AgentModel] {
        switch self {
        case .anthropic: [.sonnet5, .opus48, .haiku45]
        case .deepSeek: [.deepSeekV4Flash, .deepSeekV4Pro]
        }
    }
}

enum AgentProviderConfiguration {
    private static let providerKey = "agentProvider"

    static var selectedProvider: AgentChatProvider {
        get {
            UserDefaults.standard.string(forKey: providerKey)
                .flatMap(AgentChatProvider.init(rawValue:)) ?? .anthropic
        }
        set {
...
enum AgentModel: String, CaseIterable, Sendable {
    ...
    var provider: AgentChatProvider {
        switch self {
        case .sonnet5, .opus48, .haiku45: .anthropic
        case .deepSeekV4Flash, .deepSeekV4Pro: .deepSeek
        }
    }
...
enum AnthropicRequestBody {
    static func build(
        ...
        var messageBlocks: [[String: Any]] = messages.map {
            [
                ...
                "content": supportsImages ? $0.content : imageSafeContent($0.content),
            ]
        }
    ...
    private static func imageSafeContent(_ blocks: [[String: Any]]) -> [[String: Any]] {
        blocks.map { block in
            if block["type"] as? String == "image" {
                return ["type": "text", "text": "[Image omitted: this chat provider does not support image input.]"]
            }
            guard block["type"] as? String == "tool_result",
                  let content = block["content"] as? [[String: Any]] else { return block }
            var copy = block
            let filtered = imageSafeContent(content)
