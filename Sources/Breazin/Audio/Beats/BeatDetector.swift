...
final class BeatDetector: @unchecked Sendable {
    ...
    private static func modelURL() -> URL? {
        var candidates = [
            ...
            Bundle.main.resourceURL?.appendingPathComponent("Breazin_Breazin.bundle/Models/BeatThis.mlmodelc"),
        ].compactMap { $0 }
        if Bundle.main.bundleURL.pathExtension != "app" {
            let buildDir = Bundle(for: BeatDetectorBundleToken.self).bundleURL.deletingLastPathComponent()
            candidates.append(buildDir.appendingPathComponent("Breazin_Breazin.bundle/Models/BeatThis.mlmodelc"))
        }
