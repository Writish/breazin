    ...
    private func generatingPreview(label: String) -> some View {
        ZStack {
            ...
            VStack(spacing: AppTheme.Spacing.md) {
                ...
                if let asset = activeMediaAsset, asset.generationInput?.localJobId != nil {
                    Button("Cancel") {
        ...
        ZStack {
            ...
            VStack(spacing: AppTheme.Spacing.md) {
                ...
                Text(isUnprocessable
                    ? "Breazin loaded this clip's source file but couldn't prepare it for playback. The file may be corrupt or in an unsupported format."
                    : "Breazin couldn't load this clip's source file. It may be missing, on an ejected drive, or unreadable.")
                    .font(.system(size: AppTheme.FontSize.sm))
                    .foregroundStyle(AppTheme.Text.secondaryColor)
