...
enum AppNotifications {
    private static var enabledKey: String { "\(AppConfiguration.current.preferencesPrefix).notifications.enabled" }
    ...
    private static func body(assetName: String, assetType: ClipType, count: Int) -> String {
        if count > 1 {
            return "\(count) \(assetType.rawValue)s are ready in Breazin."
        }
